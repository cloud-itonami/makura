(ns makura.methods.social
  "social.cljc — 枕 — Foam Pillow Manufacturing DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from makura's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "枕 — Foam Pillow Manufacturing が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over makura's own domain (Foam pillow manufacturing Tier-B actor. Adopts mature global slabstock + shredding + sewing + filling methodology; religious-corp-ised by…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "makura R0: live social posting is Council Lv6+ + operator + "
